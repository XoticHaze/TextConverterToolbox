...
def normalize_deep_bar_timestamps(values: pd.Series) -> pd.Series:
    """Normalize mbytes21/MNQ_DATA Last.csv bar labels to UTC bar-start timestamps.

    Empirical source contract, verified byte-for-byte against two independent UTC
    datasets before adoption:
      * winter 2026 deep 00:01 == AXB 05:00 UTC for consecutive OHLCV rows;
      * summer 2025 deep 00:01 == licensed MNQ 04:00 UTC for consecutive OHLCV rows.

    The deep files therefore encode America/New_York local *bar-close* labels.
    Public UTC comparator datasets encode the same one-minute bars by bar start.
    Localize with DST awareness, convert to UTC, then subtract one minute.

    `ambiguous="infer"` preserves the chronological repeated hour at DST fall-back.
    `nonexistent="raise"` fails closed if a source file ever contains a local wall
    clock timestamp that did not exist at spring-forward.
    """
...
def load_deep(root: Path) -> pd.DataFrame:
    ...
    for path in files:
        ...
        nodes.append(
            frame[["timestamp", "open", "high", "low", "close", "volume", "symbol"]]
        )
