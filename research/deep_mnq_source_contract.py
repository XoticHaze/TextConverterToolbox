...
def load_deep(root: Path) -> pd.DataFrame:
    ...
    nodes=[]
    for path in sorted(root.glob("MNQ */*.Last.csv")):
        contract=path.parent.name
        if not CONTRACT_RE.fullmatch(contract): continue
        f=pd.read_csv(path)
        if list(f.columns)!=expected: raise RuntimeError(f"{path}: unexpected schema {list(f.columns)}")
        if f.empty: continue
        f["timestamp"]=normalize_deep_bar_timestamps(f["datetime"])
        for c in ["open","high","low","close","volume"]: f[c]=pd.to_numeric(f[c],errors="raise")
...
def contract_receipt() -> dict[str,str]:
    return {"source":"mbytes21/MNQ_DATA Last.csv","source_timestamp_timezone":DEEP_SOURCE_TIMEZONE,"source_bar_label":DEEP_SOURCE_BAR_LABEL,"normalized_timestamp":"UTC bar start","normalization":"tz_localize(America/New_York,DST-aware)->UTC - 1 minute"}
